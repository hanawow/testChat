		...
		if(typeof Promise !== "function") { // eslint-disable-line no-negated-condition
			...
			return new Promise(function(resolve, reject) {
				...
				try {
					...
					if(!window.$Lightning && typeof $A !== "undefined" && typeof window.$A.get === "function" && $A.get("$Site")) {
	...
	Snapins.prototype.addSessionHandlers = function addSessionHandlers() {
		...
		this.addMessageHandler("session.updateMaster", function(data) {
			if(data) {
				...
				//TODO: move this back to liveagent.esw.js when we can have multiple handlers for message type
				//      see CL 18184430
